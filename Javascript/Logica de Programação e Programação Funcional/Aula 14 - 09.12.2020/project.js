...
async function main(){
  ...
  const getText = (partialText, country) => partialText + `${country.name} in ${country.region} with ${country.population}\n`
  ...
  let text = countries.reduce(getText, '\n')
    ...
    do{
      ...
      if (option === '1'){
        ...
        const region = input.question('Takes only contries from: ')
        text = countries
